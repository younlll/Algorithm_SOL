...
void solution(int startY, int startX, int y, int x, int cnt, int dir) {
	//종료부
	if (startY == y && startX == x && cnt > 0) {
		if (maxCnt < cnt) {
			maxCnt = cnt;
		}
		return;
	...
	for (int i = 0; i < 2; i++) {
		...
		if (next_y < 0 || next_y >= n || next_x < 0 || next_x >= n) {	//다음칸이 외벽 밖일 경우, 반시계방향으로 90도 회전
			continue;
		}
		else {
			if (!chkCafe[cafeArr[next_y][next_x]]) {
				chkCafe[cafeArr[next_y][next_x]] = true;
				solution(startY, startX, next_y, next_x, cnt + 1, dir + i);
				chkCafe[cafeArr[next_y][next_x]] = false;
			}
			else if (next_y == startY && next_x == startX && cafeArr[next_y][next_x] == cafeArr[next_y][next_x]) {
				solution(startY, startX, next_y, next_x, cnt + 1, dir + i);
			}
		}
	}
...
int main() {
	...
	for (int t = 1; t <= T; t++) {
		...
		for (int i = 0; i <= n - 3; i++) {
			for (int j = 1; j <= n - 2; j++) {
				chkCafe[cafeArr[i][j]] = true;
				solution(i, j, i, j, 0, 0);
				chkCafe[cafeArr[i][j]] = false;
			}
		}
